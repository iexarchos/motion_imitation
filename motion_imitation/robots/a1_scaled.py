...
def foot_position_in_hip_frame_to_joint_angle(foot_position, scale, l_hip_sign=1):
  assert np.array(scale).shape[0]==2
  l_up = scale[0]
  l_low = scale[1]
...
@numba.jit(nopython=True, cache=True)
def foot_position_in_hip_frame(angles, scale, l_hip_sign=1):
...
@numba.jit(nopython=True, cache=True)
def analytical_leg_jacobian(leg_angles, scale, leg_id):
  """
...
# For JIT compilation
foot_position_in_hip_frame_to_joint_angle(np.random.uniform(size=3),[1.0,1.0], 1)
foot_position_in_hip_frame_to_joint_angle(np.random.uniform(size=3),[1.0,1.0], -1)


@numba.jit(nopython=True, cache=True, parallel=True)
def foot_positions_in_base_frame(foot_angles,scales):
  ...
  for i in range(4):
    if i == 0 or i == 1:
      ...
      scale = scales[2:]
    foot_positions[i] = foot_position_in_hip_frame(foot_angles[i], scale,
                                                   l_hip_sign=(-1)**(i + 1))
...
class A1(minitaur.Minitaur):
  ...
  def __init__(
    ...
    self._urdf_filename = '/motion_imitation/robots/a1_template.urdf'
  ...
  def _setDesign(self):
    if self.scales is None:
      ...
      self.SCALES = np.random.uniform(low=self.scale_range[0],high=self.scale_range[1],size=4)
    else:
  ...
  def _LoadRobotURDF(self):
    ...
    #front pair of legs 
    front_upper = self.SCALES[0]*0.2
    front_lower = self.SCALES[1]*0.2
    back_upper = self.SCALES[2]*0.2
    back_lower = self.SCALES[3]*0.2
    #link offsets
    ...
    with open(exec_dir+'/motion_imitation/robots/temp_URDFs/'+self.proc_ID+'.urdf', 'w') as file:
                file.write(filedata)
    self._pybullet_client.setPhysicsEngineParameter(enableFileCaching=0)
    a1_urdf_path = exec_dir+'/motion_imitation/robots/temp_URDFs/'+self.proc_ID+'.urdf'           
